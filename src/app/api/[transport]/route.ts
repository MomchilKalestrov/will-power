...
const formatResponse = (res: Awaited<serverActionResponse<any>>): CallToolResult => ({
    content: [ {
        type: 'text',
...
const resolveToPositionalArgs = <T extends object, K extends keyof T>(obj: T, ...order: K[]): T[ K ][] =>
    order.map(key => obj[ key ]);

const UNAUTHORIZED_ACTION = formatResponse({ success: false, reason: 'Unauthorized action.' });
...
const handler = mcp.createMcpHandler(async server => {
    Object
        ...
        .forEach(([ key, value ]) => {
            ...
            server.registerTool(
                ...
                async (args, extra) => {
                    ...
                    return formatResponse(await response);
                }
            )
        })
}, {}, {
